...
### Login i hasło do kont użytkowników
**Login:** `useri`  
**Hasło:** `passwordi`

Gdzie i to liczba naturalna z zakresu 1-50
...
- **Polecanie książek** – system sugestii dla użytkowników bazujący na ich historii wypożyczeń, preferowanych gatunkach oraz najpopularniejszych pozycjach w bibliotece.

### 3. Obsługa wypożyczeń i rezerwacji
- **Wypożyczenia książek** – rejestracja i kontrola bieżących wypożyczeń.
- **Rezerwacje książek** – możliwość rezerwowania dostępnych pozycji przez użytkowników (system kolejkowy).
- **Powiadomienia o rezerwacjach** – informowanie użytkowników o statusie ich rezerwacji, w tym automatyczne powiadomienia o dostępności książki do odbioru.

### 4. Statystyki i raporty (Panel Administratora)
- **Analityka wypożyczeń** – generowanie zestawień najczęściej wypożyczanych pozycji oraz monitorowanie trendów czytelniczych.
- **Statystyki użytkowników** – śledzenie aktywności użytkowników, wskaźników terminowości zwrotów oraz zarządzanie limitami wypożyczeń.
- **Zarządzanie zasobami** – raporty dotyczące stanu fizycznych egzemplarzy (liczba sztuk dostępnych, wypożyczonych i wyłączonych z użytku).
## Cel aplikacji
...
## 8. Endpointy rekomendacji (`/api/recommendations`)

| Metoda | Ścieżka | Uprawnienia | Opis |
|--------|---------|------------|------|
| GET | `/api/recommendations/for-user` | USER, ADMIN | Pobiera spersonalizowane rekomendacje dla użytkownika na podstawie historii. |
| GET | `/api/recommendations/similar/{bookId}` | Public | Pobiera listę książek podobnych do danej pozycji. |
| GET | `/api/recommendations/trending` | Public | Pobiera listę aktualnie najpopularniejszych książek (trendujących). |
| GET | `/api/recommendations/top-rated` | Public | Pobiera listę najwyżej ocenianych książek przez czytelników. |

---

## 9. Endpointy statystyk (`/stats`)

| Metoda | Ścieżka | Uprawnienia | Opis |
|--------|---------|------------|------|
| GET | `/stats/rentalsPerUser` | ADMIN | Pobiera listę użytkowników wraz z liczbą wypożyczonych przez nich książek (`BooksBorrowedByUserDTO`). |
| GET | `/stats/rentalsByBook` | ADMIN | Pobiera ranking popularności książek na podstawie całkowitej liczby wypożyczeń (`BookRentalsDTO`). |
| GET | `/stats/rentalsByBookThisYear` | ADMIN | Pobiera ranking popularności książek ograniczony do bieżącego roku kalendarzowego. |
...
Logika biznesowa odpowiedzialna za generowanie spersonalizowanych propozycji dla czytelników.

- `getPersonalizedRecommendations(Long userId, int limit)` – Generuje listę książek na podstawie kategorii poprzednio wypożyczonych pozycji, wykluczając te już przeczytane. Wykorzystuje algorytm punktowy łączący średnią ocen (waga 70%) i liczbę recenzji (waga 30%).
- `getSimilarBooks(Long bookId, int limit)` – Znajduje książki o tych samych kategoriach tematycznych, sortując je według ocen.
- `getTopRated(int limit)` – Zwraca listę najlepiej ocenianych książek w całej bibliotece.
- `getTrending(int limit)` – Analizuje wypożyczenia z ostatnich 30 dni, aby wyłonić najpopularniejsze "bestsellery" ostatniego miesiąca.
...
Kontroler obsługujący polecanie książek.

- `getPersonalizedRecommendations(userId, limit)` – Zwraca sugestie dopasowane do konkretnego użytkownika (wymaga logowania).
- `getSimilarBooks(bookId, limit)` – Dostarcza listę podobnych tytułów (np. do wyświetlenia na karcie szczegółów książki).
- `getTrending(limit)` – Zwraca listę pozycji, które cieszą się największym zainteresowaniem w ostatnim czasie.
- `getTopRated(limit)` – Zwraca ranking najwyżej ocenianych książek.
...
Komponent **HomePage** to główny interfejs użytkownika, który oferuje:

- Wyświetlanie pełnej listy książek w formie kart.
- Wyszukiwanie książek po tytule lub autorze.
- Filtrowanie zasobów według kategorii.
- Pobieranie okładek z Open Library API z systemem cache’owania.
- Wyświetlanie spersonalizowanych rekomendacji dla zalogowanego użytkownika.
- Prezentowanie najpopularniejszych tytułów w sekcji trendów.
- Podgląd dostępności egzemplarzy oraz średniej oceny książki.
- Dynamiczne ładowanie danych w partiach (batching) w celu optymalizacji zapytań.
- Nawigacja do szczegółów książki pod adresem /book/:id.
- Podsumowanie statystyk biblioteki w stopce strony.

![Strona Główna 1](home1.png)
![Strona Główna 2](home2.png)
![Stopka Strony](footer.png)
...
Komponent **Statistics** to panel analityczny dla administratora, który umożliwia:

- Monitorowanie kluczowych metryk – podgląd łącznej liczby użytkowników, wypożyczonych książek oraz średniej aktywności na czytelnika.
- Analizę rankingów Top 15 – wizualizacja najbardziej aktywnych czytelników oraz najpopularniejszych tytułów w formie interaktywnych wykresów słupkowych.
- Filtrowanie czasowe – porównywanie popularności książek w ujęciu historycznym oraz w bieżącym roku kalendarzowym.
- Zarządzanie widokami – szybkie przełączanie między statystykami użytkowników a zestawieniami książek za pomocą zakładek.
- Automatyczną aktualizację – pobieranie danych w czasie rzeczywistym z API przy wykorzystaniu bezpiecznej autoryzacji tokenem JWT.

![Strona Statystyk](stats.png)
