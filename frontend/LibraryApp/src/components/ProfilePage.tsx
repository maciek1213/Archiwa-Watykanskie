            ...
            <div className="max-w-7xl mx-auto">
                ...
                {!loading && !error && userData && (
                    <>
                        ...
                        {adminUser && (
                            <div className="bg-white rounded-2xl shadow-lg mb-8">
                                <div className="flex border-b border-gray-200">
                                    <button
                                        className={`px-6 py-4 text-lg font-medium ${activeAdminTab === 'profile' ? 'text-amber-700 border-b-2 border-amber-700' : 'text-gray-500 hover:text-gray-700'}`}
                                        onClick={() => setActiveAdminTab('profile')}
                                    >
                                        Profil użytkownika
                                    </button>
                                    <button
                                        className={`px-6 py-4 text-lg font-medium ${activeAdminTab === 'users' ? 'text-amber-700 border-b-2 border-amber-700' : 'text-gray-500 hover:text-gray-700'}`}
                                        onClick={() => setActiveAdminTab('users')}
                                    >
                                        Zarządzanie użytkownikami
                                    </button>
                                    <button
                                        className={`px-6 py-4 text-lg font-medium ${activeAdminTab === 'books' ? 'text-amber-700 border-b-2 border-amber-700' : 'text-gray-500 hover:text-gray-700'}`}
                        ...
                        {/* Profile Tab Content */}
                        {(activeAdminTab === 'profile' || !adminUser) && (
                            <>
                                {/* Profil użytkownika */}
                      ...
                      {bookQueues.length} w kolejce
                        ...
                        {adminUser && activeAdminTab === 'users' && (
                            <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                                    <div>
                                        <h3 className="text-2xl font-bold text-gray-900">Zarządzanie Użytkownikami</h3>
                                        <p className="text-gray-600 mt-2">
                                            Znaleziono: {filteredUsers.length} użytkowników (wyświetlanie {indexOfFirstUser + 1}-{Math.min(indexOfLastUser, filteredUsers.length)})
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            <input
                                                ...
                                                className="pl-10 pr-4 py-2 w-full md:w-64 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors"
                                                ...
                                                if (currentToken) {
                                                    const fetchAllUsers = async () => {
                                                        try {
                                                            ...
                                                            console.error("Błąd podczas odświeżania użytkowników", err);
                                                        } finally {
                                                            setLoadingUsers(false);
                                                        }
                                                    };
                                        ...
                                        {totalUserPages > 1 && (
                                            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-6 pt-6 border-t border-gray-200">
                                                <div className="text-gray-600">
                                                    Strona {userCurrentPage} z {totalUserPages}
                                                </div>
                                                <div className="flex gap-2">
                                                    <button
                                                        className={`px-4 py-2 rounded-lg ${userCurrentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                                    ...
                                                    {Array.from({ length: Math.min(5, totalUserPages) }, (_, i) => {
                                                        ...
                                                        return (
                                                            <button
                                                                key={pageNum}
                                                                className={`px-4 py-2 rounded-lg ${userCurrentPage === pageNum
                                                    ...
                                                    <button
                                                        className={`px-4 py-2 rounded-lg ${userCurrentPage === totalUserPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
