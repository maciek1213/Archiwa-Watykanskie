...
export function BookManagement({ token }: Props) {
    ...
    const [filteredItems, setFilteredItems] = useState<BookItem[]>([]);
        ...
        if (bookSearchTerm) {
            const filtered = books.filter(book =>
                ...
                book.categories.some(cat =>
                    cat.name.toLowerCase().includes(bookSearchTerm.toLowerCase())
                )
            );
    ...
    useEffect(() => {
        if (itemSearchTerm) {
            const filtered = bookItems.filter(item =>
                item.isbn.toLowerCase().includes(itemSearchTerm.toLowerCase())
            );
    ...
    }, [itemSearchTerm, bookItems]);

    // Calculate pagination
    const indexOfLastBook = bookCurrentPage * booksPerPage;
    const indexOfFirstBook = indexOfLastBook - booksPerPage;
    const currentBooks = filteredBooks.slice(indexOfFirstBook, indexOfLastBook);
    const totalBookPages = Math.ceil(filteredBooks.length / booksPerPage);

    const indexOfLastItem = itemCurrentPage * itemsPerPage;
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    ...
                    <p className="text-gray-600 mt-2">Zarządzanie książkami, kategoriami i egzemplarzami</p>
                </div>
            ...
            <div className="mb-12">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    <h4 className="text-xl font-bold text-gray-900">Lista książek ({filteredBooks.length})</h4>
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <input
                                type="text"
                                placeholder="Szukaj książki..."
                                className="pl-10 pr-4 py-2 w-full md:w-64 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                        ...
                        <button
                            className="px-4 py-2 bg-gradient-to-r from-amber-600 to-amber-700 text-white font-semibold rounded-lg hover:from-amber-700 hover:to-amber-800 transition-colors"
                            onClick={fetchBooks}
                        >
                            Odśwież
                        </button>
                ...
                {loading ? (
                    ...
                    <>
                        <div className="overflow-hidden rounded-xl border border-gray-200 shadow mb-6">
                                                ...
                                                <div className="flex flex-wrap gap-2 max-w-xs">
                                                    {book.categories && book.categories.length > 0 ? (
                                                        book.categories.map((category) => (
                                                            <span
                                                                key={category.id}
                                                                className="px-3 py-1 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-full text-xs font-semibold truncate max-w-[120px]"
                                                                title={category.name}
                                                            >
                                                                {category.name}
                                                            </span>
                        ...
                        {totalBookPages > 1 && (
                            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-4">
                                <div className="text-gray-600">
                                    Strona {bookCurrentPage} z {totalBookPages} ({filteredBooks.length} książek)
                                </div>
                                <div className="flex gap-2">
                                    <button
                                        className={`px-4 py-2 rounded-lg ${bookCurrentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                        onClick={() => setBookCurrentPage(prev => Math.max(prev - 1, 1))}
                                        disabled={bookCurrentPage === 1}
                                    >
                                        Poprzednia
                                    </button>
                                    {Array.from({ length: Math.min(5, totalBookPages) }, (_, i) => {
                                        let pageNum;
                                        if (totalBookPages <= 5) {
                                            pageNum = i + 1;
                                        } else if (bookCurrentPage <= 3) {
                                            pageNum = i + 1;
                                        } else if (bookCurrentPage >= totalBookPages - 2) {
                                            pageNum = totalBookPages - 4 + i;
                                        } else {
                                            pageNum = bookCurrentPage - 2 + i;
                                        }
                                        return (
                                            <button
                                                key={pageNum}
                                                className={`px-4 py-2 rounded-lg ${bookCurrentPage === pageNum
                                    ...
                                    <button
                                        className={`px-4 py-2 rounded-lg ${bookCurrentPage === totalBookPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                        onClick={() => setBookCurrentPage(prev => Math.min(prev + 1, totalBookPages))}
                                ...
                                <div className="text-sm text-gray-500">
                                    {booksPerPage} książek na stronę
                                </div>
                            </div>
                        )}
                    </>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                        <div>
                            ...
                            <p className="text-gray-600 mt-2">
                                Znaleziono: {filteredItems.length} egzemplarzy
                            </p>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="relative">
                        ...
                        <>
                          ...
                          <span className={`px-4 py-2 inline-flex text-sm leading-5 font-semibold rounded-full ${item.isAvailable
                            ...
                            {totalItemPages > 1 && (
                                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-4">
                                    <div className="text-gray-600">
                                        Strona {itemCurrentPage} z {totalItemPages}
                                    </div>
                                    <div className="flex gap-2">
                                        <button
                                            className={`px-4 py-2 rounded-lg ${itemCurrentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                        ...
                                        {Array.from({ length: Math.min(5, totalItemPages) }, (_, i) => {
                                            ...
                                            if (totalItemPages <= 5) {
                                                pageNum = i + 1;
                                            } else if (itemCurrentPage <= 3) {
                                                pageNum = i + 1;
                                            } else if (itemCurrentPage >= totalItemPages - 2) {
                                                pageNum = totalItemPages - 4 + i;
                                            } else {
                                                pageNum = itemCurrentPage - 2 + i;
                                            }
                                            return (
                                                <button
                                                    key={pageNum}
                                                    className={`px-4 py-2 rounded-lg ${itemCurrentPage === pageNum
                                                        ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white'
                                                        : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                                    }`}
                                                    onClick={() => setItemCurrentPage(pageNum)}
                                                >
                                                    {pageNum}
                                                </button>
                                            );
                                        ...
                                        <button
                                            className={`px-4 py-2 rounded-lg ${itemCurrentPage === totalItemPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                    ...
                                    <div className="text-sm text-gray-500">
                                        {itemsPerPage} egzemplarzy na stronę
                                    </div>
                                </div>
                            )}
                        </>
            ...
            <div className="mt-12 p-8 bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-2xl border border-emerald-200 shadow-lg">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        <h3 className="text-3xl font-bold text-gray-900 mb-2">Zarządzanie Kategoriami</h3>
                        <p className="text-gray-600">Zarządzanie kategoriami książek</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <input
                                ...
                                className="pl-10 pr-4 py-2 w-full md:w-64 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                ...
                {filteredCategories.length === 0 ? (
                    ...
                    <>
                        ...
                        {totalCategoryPages > 1 && (
                            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-4">
                                <div className="text-gray-600">
                                    Strona {categoryCurrentPage} z {totalCategoryPages} ({filteredCategories.length} kategorii)
                                </div>
                                <div className="flex gap-2">
                                    <button
                                        className={`px-4 py-2 rounded-lg ${categoryCurrentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                    ...
                                    {Array.from({ length: Math.min(5, totalCategoryPages) }, (_, i) => {
                                        ...
                                        return (
                                            <button
                                                key={pageNum}
                                                className={`px-4 py-2 rounded-lg ${categoryCurrentPage === pageNum
                                    ...
                                    <button
                                        className={`px-4 py-2 rounded-lg ${categoryCurrentPage === totalCategoryPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                ...
                                <div className="text-sm text-gray-500">
                                    {categoriesPerPage} kategorii na stronę
                                </div>
                            </div>
                        )}
                    </>
