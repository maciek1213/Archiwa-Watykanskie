    ...
    const fetchSimilarBooks = async () => {
        ...
        try {
            const response = await axios.get<Book[]>(
                `http://localhost:8080/api/recommendations/similar/${id}?limit=4`,
                {
                    headers: {
                        Authorization: `Bearer ${effectiveToken}`,
                    },
            ...
            console.error("Nie udało się pobrać podobnych książek", err);
        }
    };
                                ...
                                <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-lg">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                        {similarBooks.map((similarBook) => (
                                            <div
                                                key={similarBook.id}
                                                className="group bg-white rounded-xl border border-gray-200 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer hover:-translate-y-2"
                                                onClick={() => navigate(`/book/${similarBook.id}`)}
                                            >
                                                <div className="relative h-40 overflow-hidden bg-gradient-to-br from-amber-100 to-amber-200 flex items-center justify-center">
                                                    <svg className="w-12 h-12 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                                                    </svg>
                                                </div>
                                                <div className="p-4">
                                                    <h5 className="text-sm font-bold text-gray-900 mb-1 line-clamp-2 group-hover:text-amber-700 transition-colors">
                                                        {similarBook.title}
                                                    </h5>
                                                    <p className="text-xs text-amber-600 mb-2">{similarBook.author}</p>
                                                    {similarBook.reviewCount && similarBook.reviewCount > 0 && (
                                                        <div className="flex items-center gap-1">
                                                            <div className="flex items-center">
                                                                {[1, 2, 3, 4, 5].map((star) => (
                                                                    <svg
                                                                        key={star}
                                                                        className={`w-3 h-3 ${
                                                                            star <= Math.round(similarBook.averageRating || 0)
                                                                                ? 'text-yellow-400 fill-current'
                                                                                : 'text-gray-300'
                                                                        }`}
                                                                        fill="none"
                                                            ...
                                                            <span className="text-xs text-gray-600">{similarBook.averageRating?.toFixed(1)}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
